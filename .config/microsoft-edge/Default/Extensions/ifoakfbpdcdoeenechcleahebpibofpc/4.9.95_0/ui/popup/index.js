(function () {
    ...
    function ThemeSetupPage(props) {
        ...
        const moveHslSaturation = (hsl, x) => ({
            ...hsl,
            s: clamp(hsl.s + x, 0, 1)
        });
        const moveHslLightness = (hsl, x) => ({
            ...hsl,
            l: clamp(hsl.l + x, 0, 1)
        });
        const mixRgb = (a, b) =>
            Object.keys(a).reduce(
                (obj, key) =>
                    Object.assign(obj, {[key]: (a[key] + b[key]) / 2}),
                {}
            );
        const bgPalette = (() => {
                    ...
                    m$1(
                        ...
                        m$1(
                            ...
                            bgPalette.map((background) => {
                                const rgb = parseColorWithCache(background);
                                const lightness = getSRGBLightness(
                                    rgb.r,
                                    rgb.g,
                                    rgb.b
                                );
